...
$data, $num_seq, $nsu, $num_autorizacao, $parcelas, $transacao, $total, $filiacao, $distribuidor, $id_pedido, $pax1, $pax2, $pax3, $pax4, $numdoc1, $numdoc2, $numdoc3, $numdoc4
) {
    $args = array(
        "Data" => $data,
        "NumSqn" => $num_seq,
...
$filiacao, $usr, $pwd, $distribuidor, $data_inicial, $data_final, $tipo_trx, $status_trx, $servico_avs
) {
    $args = array(
        "Filiacao" => $filiacao,
        "Distribuidor" => $distribuidor,
...
$filiacao, $total, $parcelas, $data, $num_autorizacao, $nsu, $concentrador, $usr, $pwd
) {
    $args = array(
        "Filiacao" => $filiacao,
        "Total" => $total,
...
$filiacao, $distribuidor, $total, $parcelas, $data, $num_autorizacao, $nsu, $concentrador, $usr, $pwd
) {
    $args = array(
        "Filiacao" => $filiacao,
        "Distribuidor" => $distribuidor,
...
$filiacao, $distribuidor, $total, $data, $num_autorizacao, $nsu, $concentrador, $usr, $pwd
) {
    $args = array(
        "Filiacao" => $filiacao,
        "Distribuidor" => $distribuidor,
...
function raw_get_authorized_avs($total, $transacao, $parcelas, $filiacao, $id_pedido, $cartao, $cvc, $mes, $ano, $portador, $cpf, $endereco, $num1, $complemento, $cep1, $cep2, $iata, $distribuidor, $concentrador, $taxa_embarque, $entrada, $numdoc1, $numdoc2, $numdoc3, $numdoc4, $add_data, $pax1, $pax2, $pax3, $pax4) {
    $args = array(
        "Filiacao" => $filiacao,
        "Total" => $total,
        "Transacao" => $transacao,
 ...
 * @param type $nsu
 * @param type $num_autorizacao
 * @param type $total
 * @param type $filiacao
 * @param type $usr
 * @param type $pwd
 * @param type $concentrador
 * @return type
 * 
 *  TOTAL           10 Valor total da compra
 *  FILIACAO        9 Número de filiação do estabelecimento
 *  NUMCV           9 Número do Comprovante de Venda (NSU)
 *  NUMAUTOR        6 Número de Autorização
 *  CONCENTRADOR    5 N/A – Enviar parâmetro com valor vazio
 *  USR             16 Código do usuário Komerci
 *  PWD             20 Senha de acesso do usuário Komerci
 */
function raw_void_transaction($nsu, $num_autorizacao, $total, $filiacao, $usr, $pwd, $concentrador) {
    $args = array(
        "Total" => $total,
        "Filiacao" => $filiacao,
        "NumCV" => $nsu,
...
function raw_get_authorized($filiacao, $total, $transacao, $parcelas, $id_pedido, $cartao, $cvc, $mes, $ano, $portador, $conftxn, $iata, $distribuidor, $concentrador, $taxa_embarque, $entrada, $pax1, $pax2, $pax3, $pax4, $numdoc1, $numdoc2, $numdoc3, $numdoc4, $add_data) {
    $args = array(
        "Filiacao" => $filiacao,
        "Total" => $total,
        "Transacao" => $transacao,
...
function paga_direto($id_pedido, $total, $parcelas, $cartao, $cvc, $mes, $ano, $portador) {
    ...
    list($transacao, $parcelas) = ($parcelas <= 1) ?
            array("04", "00") : // a vista
            array(// parcelado
    ...
    $result = raw_get_authorized(
            $_komerci_config["filiacao"], number_format($total, 2, ".", ""), $transacao, $parcelas, $id_pedido, preg_replace("/[^0-9]/", "", $cartao), preg_replace("/[^0-9]/", "", $cvc), ($mes + 0 < 10) ? "0" . $mes : $mes, ($ano + 0), strtoupper(remove_accents($portador)), ($_komerci_config["duas_etapas"] ? "N" : "S"), "", "", "", "", "", "", "", "", "", "", "", "", "", ""
    );
    ...
    $result_txn = raw_confirm_txn(
            $result["DATA"], $result["NUMSQN"], $result["NUMCV"], $result["NUMAUTOR"], $parcelas, $transacao, number_format($total, 2, ".", ""), $_komerci_config["filiacao"], "", $id_pedido, "", "", "", "", "", "", "", ""
    );
...
function estorna($nsu, $num_autorizacao, $total) {
    ...
    return raw_void_transaction(
            $nsu, $num_autorizacao, number_format($total, 2, ".", ""), $_komerci_config["filiacao"], $_komerci_config["usuario"], $_komerci_config["senha"], ""
    );
}
...
function http_read($url, $raw_post_data) {
    ...
    if (count($_SESSION["_komerci_cookie_"])) {
        $array = array_map(
                create_function('$k, $v', 'return "$k=$v";'), array_keys($_SESSION["_komerci_cookie_"]), array_values($_SESSION["_komerci_cookie_"])
        );
